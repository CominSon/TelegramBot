...
class AbilityBotExample(
    ...
    fun replyMessageMainMenu(): Reply {
        val action: (BaseAbilityBot, Update) -> Unit = { _, upd ->
            ...
            msg = sendInlineKeyboard(upd.message.chatId.toString())
            sender.execute(msg)
        }
    ...
    fun replyMessageListDictionaries(): Reply {
        val action: (BaseAbilityBot, Update) -> Unit = { _, upd ->
            var msg = SendMessage(
                upd.message.chatId.toString(),
                "${upd.message.from.firstName}, выберите словарь, который хотите добавить:"
            )
            //Удалить клавиатуру
            msg.replyMarkup = ReplyKeyboardRemove(true)
            ...
            //здесь переход в список всех словарей (пока эта функция в качестве демонстрации)
            msg = sendInlineKeyboard(upd.message.chatId.toString())
            sender.execute(msg)
        }
    ...
    fun replyMessageCreateNewDictionary(): Reply {
        val action: (BaseAbilityBot, Update) -> Unit = { _, upd ->
            var msg = SendMessage(
                upd.message.chatId.toString(),
                "${upd.message.from.firstName}, введите название нового словаря:"
            )
            //Удалить клавиатуру
            msg.replyMarkup = ReplyKeyboardRemove(true)
    ...
    fun replyMessageDeleteDictionaryFromFavorites(): Reply {
        val action: (BaseAbilityBot, Update) -> Unit = { _, upd ->
            var msg = SendMessage(
                upd.message.chatId.toString(),
                "${upd.message.from.firstName}, нажмите на словарь, который хотите удалить из списка изучаемых: "
            )
            //Удалить клавиатуру
            msg.replyMarkup = ReplyKeyboardRemove(true)
